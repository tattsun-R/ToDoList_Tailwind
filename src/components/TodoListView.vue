...
<tbody>
    ...
    >
        ...
        <td class="rounded px-2 py-1" >
            ...
            <input v-else v-model="inputContent" type="text" class="w-full p-1 border-2 border-sky-200 rounded"  />
        </td>
        <td class="px-2 py-1">
            <span v-if="!item.onEdit">{{ item.limit }}</span>
            <input v-else v-model="inputLimit" type="date" class="w-full p-1 border-2 border-sky-200 rounded"  />
        </td>
        <td class="px-2 py-1">
            ...
            <select v-else v-model="inputState" class="w-full p-1 border-2 border-sky-200 rounded" >
               <option
        ...
        <td  class="bg-sky-500 text-white font-semibold shadow-sm px-1 py-1 rounded-lg" >
            <button v-if="!item.onEdit" @click="onEdit(item.id)">編集</button>
            <button v-else @click="onUpdate(item.id)">完了</button>
        </td>
        <td class="bg-white text-gray-700 font-semibold shadow-sm px-1 py-1 ring-1 rounded-lg" ><button @click="showDeleteModal(item.id)">削除</button></td>
    </tr>
</tbody>
    </table>
</div>

    <div v-if="isShowModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity flex items-center justify-center">
        <div class="bg-white rounded-lg shadow-lg p-6 text-center max-w-md w-full">
            <div>
                <div class="inline-flex">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                    <p class="ml-2 mb-4">{{ deleteItemContent }}を削除してもよろしいですか？</p>
                </div>
                <div class="inline-flex justify-center">
                    <button @click="onDeleteItem()" class="rounded-md bg-sky-500 px-3 py-2 mr-2 text-sm font-semibold text-white shadow-sm hover:bg-sky-400">はい</button>
                    <button @click="onHideModal()" class="rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">キャンセル</button>
                </div>
            </div>
        </div>
    </div>
